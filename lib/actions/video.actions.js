...
export const FetchVideosbyTags = async ({ tag, nCursor, maxResults }) => {
    ...
    try {
        ...
        const publicIdAndUrls = results.resources.map(resource => {
            ...
            const modifiedUrl = resource.secure_url.replace(/\.(mov|mp4|webm)$/, '.jpg');

            // Return an object with both secure_url and public_id
            return {
                secure_url: modifiedUrl,
                public_id: resource.public_id,
                secure_b:resource.secure_url
...
export const AddTagsActionVideo = async ({ selectedSize, pId }) => {
    console.log(selectedSize, pId)
...
export const RemoveTagsVideo = async ({ pId }) => {
    console.log('RemoveTagsVideo', RemoveTagsVideo)
    cloudinary.config({
        cloud_name: process.env.CLOUDINARY_CLOUD_NAME,
        api_key: process.env.CLOUDINARY_API_KEY,
        api_secret: process.env.API_SECRET,
