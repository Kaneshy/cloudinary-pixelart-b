...
export const FetchImagesbyTags = async ({ tag, nCursor, maxResults }) => {
  console.log(tag)
  cloudinary.config({
    cloud_name: process.env.CLOUDINARY_CLOUD_NAME,
    api_key: process.env.CLOUDINARY_API_KEY,
  ...
  try {
    ...
    return {
      props: {
        publicIdAndUrls: publicIdAndUrls,
        publicId: publicId, // Pasar secureUrls como propiedad al componente homePage
...
export const RemoveTags = async ({ pId }) => {
  cloudinary.config({
    cloud_name: process.env.CLOUDINARY_CLOUD_NAME,
    api_key: process.env.CLOUDINARY_API_KEY,
    api_secret: process.env.API_SECRET,
  ...
  try {
    // Construct the base expression
    let expression = `resource_type:image AND folder=books AND public_id=${pId}`;
    // Conditionally add the tag filter
    // if (tag !== 'All') {
    //     expression += ` AND tags=${tag}`;
    // }
...
export const AddTagsAction = async ({ selectedSize, pId }) => {
  console.log('ad', selectedSize, pId)
  ...
  try {
    ...
    // Execute the search query
    const resp = await cloudinary.v2.uploader
      .add_tag(selectedSize,
        pId)
      .then(result => console.log(result));
